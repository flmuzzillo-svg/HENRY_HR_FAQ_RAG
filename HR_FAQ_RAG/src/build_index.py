...
Este script implementa el pipeline completo de indexacion:
  ...
  3. Generacion de embeddings con OpenAI text-embedding-3-large
...
# Modelo de embeddings (OpenAI, segun las restricciones del prompt)
...
def create_embeddings() -> OpenAIEmbeddings:
    ...
    Usa el modelo text-embedding-3-large, que ofrece vectores de alta calidad
    para busqueda semantica en espanol.

    Returns:
        Instancia configurada de OpenAIEmbeddings.

    Raises:
        EnvironmentError: Si OPENAI_API_KEY no esta configurada.
    """
    api_key = os.getenv("OPENAI_API_KEY", "")
    if not api_key or api_key.startswith("sk-REEMPLAZA"):
    ...
    print(f"  [Embed] Modelo de embeddings: {EMBEDDING_MODEL}")
    return embeddings
