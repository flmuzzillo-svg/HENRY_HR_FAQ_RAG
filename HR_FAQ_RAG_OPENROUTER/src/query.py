...
Nota: Este modulo usa OpenRouter como proveedor de API (compatible con OpenAI).
...
# Modelo de generacion (via OpenRouter, compatible con modelos OpenAI y otros)
...
def generate_answer(
    ...
    """
    Genera una respuesta anclada al contexto usando un LLM via OpenRouter.
    ...
    Args:
        ...
        model: Nombre del modelo a utilizar (formato OpenRouter: proveedor/modelo).
...
Responde basandote UNICAMENTE en el contexto proporcionado."""
    ...
    print(f"  [Generate] Respuesta generada ({len(response.content)} caracteres) via OpenRouter")
